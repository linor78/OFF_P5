...
S1_URL = 'https://fr.openfoodfacts.org/cgi/search.pl?&\
tagtype_0=languages&tag_contains_0=contains&tag_0=fr&tagtype_1=\
categories&tag_contains_1=contains&tag_1='
...
class controller():
    ...
    def return_values(self, category, jsond):
        # Function that return the list of all the values we want to save in
        # our database for every product from its json object
        values = list()
        value = None
            ...
            for product in jsond:
                try:
                    if product['stores'] and product['nutrition_grade_fr']:
                        self.products.add_product(self.return_values
                                                  (category, product))
                except KeyError:
                    pass
            i += 1
    ...
    def insert_into_mysubstituts(self, product_ean, sub_ean):
        ...
        query = 'insert into mysubstituts (EAN,Origin) values ('
        query = query + str(sub_ean)
        query = query + ', ' + str(product_ean) + ');'
        try:
            self.db.curs.execute(query)
        except mysql.connector.Error as err:
            print("Something went wrong: {}".format(err))
